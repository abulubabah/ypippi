	...
	elseif($akses=="admin" or $akses=="super"){ 
		...
		if ($paket=="free") { ?>
			...
			if ($aktif==0) { ?><h3>Fitur Tidak Aktif</h3>Maaf, Fitur ini belum aktif, karena Anda belum melakukan Pembayaran.<?php }
				...
				$query=mysqli_query($koneksi, "SELECT judul,isi,gambar FROM psb_halaman WHERE subdomain='$subdomain' AND link='alur'");
				...
				elseif ($_POST['proses']=="edit") {
					if (empty($_SESSION['kode'])) {?>
						<h3>Alur Pendaftaran Berhasil Disimpan</h3>Selamat, Alur Pendaftaran Berhasil Disimpan<br/><a href="<?php echo $module->getHttp();?>://<?php echo $linkhalaman;?>" title="Kembali">Kembali</a><?php
					}
					else {
						if ($_FILES['gambar']['tmp_name']=="") {
							...
							if ($jumlah==1) { mysqli_query($koneksi, "UPDATE psb_halaman SET judul='$judul', isi='$isi' WHERE subdomain='$subdomain'  AND link='alur'"); }
							else { mysqli_query($koneksi, "INSERT INTO psb_halaman (subdomain, judul, link, isi, tgl) VALUES ('$subdomain', '$judul', 'alur', '$isi', sysdate())"); } ?>
							<h3>Alur Pendaftaran Berhasil Disimpan</h3>Selamat, Alur Pendaftaran Berhasil Disimpan<br/><a href="<?php echo $module->getHttp();?>://<?php echo $linkhalaman;?>" title="Kembali">Kembali</a><?php
						} 
						else {
							...
							elseif ($header_dimensi['0']>"1000" or $header_dimensi['1']>"1000") { ?>
								...
								$judul=strip_tags($_POST['judul']); $judul=str_replace('"','',$judul); $judul=str_replace("'","",$judul);
								$isi=str_replace("'","",$_POST['isi']); $isi=str_replace('"','',$isi);
								if ($jumlah==1) { mysqli_query($koneksi, "UPDATE psb_halaman SET judul='$judul', isi='$isi', gambar='$judul_baru' WHERE subdomain='$subdomain'  AND link='alur'"); }
								else { mysqli_query($koneksi, "INSERT INTO psb_halaman (subdomain, judul, link, isi, gambar, tgl) VALUES ('$subdomain', '$judul', 'alur', '$isi', '$judul_baru', sysdate())"); }
