	...
	elseif($akses=="publik" or $akses=="member"){ 
		...
		if ($no=="") { 					
			if ($link=="" or $link=="home" or $link=="beranda") { 
				...
				<h2><a href="//<?php echo $linksub."/".$link;?>/" title="Pengumuman">Pengumuman</a></h2><?php 
			}
			$haltipe="";
			$halidtipe="";
			$ppdb->artikel_list($subdomain,$linksub,$bahasa,$tabel,$haltipe,$halidtipe);
		}
		else {
			$no=$no/1;
			$ppdb->artikel_view ($subdomain,$linksub,$bahasa,$tabel,$no);
