...
function contact_list($linksub,$subdomain,$link) {
	$mod=$link;
	$uname=$_SESSION['uname'];
	$linkcreate="$linksub/$mod/create/";
	$linkdetail="$linksub/$mod/detail";
	?> 
	<div style="display:table; width:100%">
		<div style="float:left"><h2>Hubungi Admin</h2></div>
		<div class="button" style="float:right; padding:1px 3px;"><a href="//<?php echo $linkcreate;?>" title="Kirim Pesan">Kirim Pesan</a></div>
	</div>
	<div class="bar"><?php
	$query=mysqli_query($koneksi, "SELECT *,DATE_FORMAT(tgl,'%d-%m-%Y %H:%i:%s') as tanggal FROM psb_contact WHERE iduser='$uname' AND subdomain='$subdomain'  AND status!='closed' ORDER BY tgl DESC");
	$jumlah=mysqli_num_rows($query); 
	if ($jumlah==0) { ?>Anda Tidak Memilik Status Pesan Dengan Admin<br>Mulai <a href="//<?php echo $linkcreate;?>" title="Kirim Pesan">Kirim Pesan</a> Ke Admin<?php }
	else { ?>
		...
		while($data=mysqli_fetch_array($query)) { 
				...
				<tr bgcolor="<?php echo $latar; ?>">
					...
					<td style="color:<?php echo $warna;?>; font-weight:<?php echo $bold;?>;"><a href="//<?php echo $linkdetail."/".$no;?>" title="Detail">Detail</a></td>
				</tr><?php
				$nomor++; $y++;
		} ?>
		</table><?php
...
function contact_create ($linksub,$subdomain,$link,$folder) { 
		...
		if (empty($_SESSION['kode'])) {  ?><h3>Session Habis</h3>Maaf,Session Telah Habis<br/><a href="" onclick="document.location;history.go(-1);" title="Kembali">Kembali</a> <?php	}
			...
			elseif ($isi=="") { ?><div class="warning"><h3>Isi Pesan Masih Kosong</h3>Isi Pesan Harus Diisi. Silahkan Ulangi Lagi<br><a href="" onclick="document.location;history.go(-1);" title="Kembali">Kembali</a></div><?php }
				...
				if (empty($_FILES['gambar']['tmp_name'])) {
					mysqli_query($koneksi, "INSERT INTO psb_contact (iduser,subdomain, judul, status, tgl) VALUES ('$idmember','$subdomain', '$judul', 'open', sysdate())");
					$qlast=mysqli_query($koneksi, "SELECT * FROM psb_contact WHERE iduser='$uname' ORDER BY tgl DESC");
					$dlast=mysqli_fetch_array($qlast);
					$id_contact=$dlast['no'];
					mysqli_query($koneksi, "INSERT INTO psb_contact_isi (id_contact,subdomain, iduser, isi, tgl) VALUES ('$id_contact','$subdomain', '$idmember', '$isi', sysdate())");?>
					...
					if ($gambar_type!="image/jpg" and $gambar_type!="image/png" and $gambar_type!="image/gif" and $gambar_type!="image/jpeg"){  ?>
						...
						mysqli_query($koneksi, "INSERT INTO psb_contact (iduser,subdomain judul, status, tgl) VALUES ('$idmember','$subdomain', '$judul', 'open', sysdate())");
						$qlast=mysqli_query($koneksi, "SELECT * FROM psb_contact WHERE iduser='$uname' and subdomain='$subdomain' ORDER BY tgl DESC");
						$dlast=mysqli_fetch_array($qlast);
						$id_contact=$dlast['no'];
						mysqli_query($koneksi, "INSERT INTO psb_contact_isi (id_contact,subdomain, iduser, isi, gambar, tgl) VALUES ('$id_contact','$subdomain', '$idmember', '$isi', '$judul_baru', sysdate())");?>
...
function contact_detail($linksub,$subdomain,$link,$folder,$id_contact) {
	$mod=$link;
	$picture=$folder;
	$linkpic="$picture/picture";
	$uname=$_SESSION['uname'];
	//variabel get no= variabel get linkhal
	if (empty ($id_contact)) { ?> <h3>Session Habis</h3>Maaf,Session Telah Habis<br/><a href="" onclick="document.location;history.go(-1);" title="Kembali">Kembali</a> <?php }
	else { 
		$id_contact=$id_contact/1;
		$qcontact=mysqli_query($koneksi, "SELECT judul FROM psb_contact WHERE no='$id_contact'");
		$dcontact=mysqli_fetch_array($qcontact);
		$judul=$dcontact['judul']; ?>
		<h2>Detail Pesan : <?php echo $judul;?></h2><?php
		$nomor=1;
		$y=1;
		$query=mysqli_query($koneksi, "SELECT *,DATE_FORMAT(tgl,'%d-%m-%Y | %H:%i:%s') as tanggal FROM psb_contact_isi WHERE id_contact='$id_contact' ORDER BY tgl ASC");
		while($data=mysqli_fetch_array($query)) { 
			$no=$data['no'];
			$tanggal=$data['tanggal'];
			$isi=$data['isi'];
			$iduser=$data['iduser'];
			$gambar=$data['gambar'];
			$qmem=mysqli_query($koneksi, "SELECT nama FROM psb_member WHERE email='$iduser'");
			$dmem=mysqli_fetch_array($qmem);	
			if ($iduser=="admin") { $namamem="Admin $subdomain"; } else { $namamem=$dmem['nama']." [".$iduser."]"; }
			if ($gambar=="") { $linkgambar=""; } else { $linkgambar="<a href=\"http:$linkpic/$gambar\" target=\"_blank\">Download Picture</a>"; }
			if ($iduser=="admin") { $latar="#88EEFF"; } else { $latar="#EAEAEA";} ?>
...
function contact_reply ($linksub,$subdomain,$link,$folder) { 
	$mod=$link;
	$uname=$_SESSION['uname'];
	$folder=$folder;
	$folderpicture="$folder/picture/";
	...
	elseif ($_POST['process']=="reply") {
		if (empty($_SESSION['kode'])) {?>  <h3>Session Habis</h3>Maaf,Session Telah Habis<br/><a href="" onclick="document.location;history.go(-1);" title="Kembali">Kembali</a> <?php	}
			...
			if ($isi=="" or $isi=="Tulis Komentar...") { ?><div class="warning"><h3>Isi Masih Kosong</h3>Silahkan Ulagi Lagi<br><a href="" onclick="document.location;history.go(-1);" title="Kembali">Kembali</a> </div><?php }
				...
				if (empty($_FILES['gambar']['tmp_name'])) {
					mysqli_query($koneksi, "INSERT INTO psb_contact_isi (id_contact,subdomain, iduser, isi, tgl) VALUES ('$id_contact','$subdomain','$idmember', '$isi', sysdate())");
					mysqli_query($koneksi, "UPDATE psb_contact SET status='reply', tgl=sysdate() WHERE no='$id_contact'"); ?>
					<div class="success"><h3>Pesan Anda Telah Terkirim</h3>Tunggu Informasi / Balasan Dari Admin<br><a href="" onclick="document.location;history.go(-1);" title="Kembali">Kembali</a> </div><?php
				}
				else {
					$gambar=$_FILES['gambar']['tmp_name'];
...
function contact_closed ($linksub,$subdomain,$link) { 
	$mod=$link;
	$uname=$_SESSION['uname'];
	...
	elseif ($_POST['process']=="closed") {
		if (empty($_SESSION['kode'])) {  $member->notify("empty_session");	}
			...
			mysqli_query($koneksi, "UPDATE psb_contact SET status='closed', tgl=sysdate() WHERE no='$id_contact'"); ?>
...
function contact_adminlist ($linksub,$subdomain,$act) { 
	$admin=new admin ();
	$admin->get_variable();
	...
	$y=1;
	$totbantuan=0;
	$query=mysqli_query($koneksi, "SELECT *,DATE_FORMAT(tgl,'%d-%m-%Y %H:%i:%s') as tanggal FROM psb_contact WHERE  subdomain='$subdomain' ORDER BY tgl DESC");
	while($data=mysqli_fetch_array($query)) { 
			$no=$data['no'];
			$tanggal=$data['tanggal'];
			...
			$qmem=mysqli_query($koneksi, "SELECT nama FROM psb_member WHERE email='$iduser' ");
			$dmem=mysqli_fetch_array($qmem);
			$namamember=$dmem['nama'];
			$qcontactisi=mysqli_query($koneksi, "SELECT isi FROM psb_contact_isi WHERE id_contact='$no' AND subdomain='$subdomain' ORDER BY tgl ASC");
			$dcontactisi=mysqli_fetch_array($qcontactisi);
			$isi=$dcontactisi['isi'];
			if ($y%2==0) { $latar="#F0F0F0"; } else { $latar="#FFFFFF"; } ?>
			<tr bgcolor="<?php echo $latar; ?>">
				<td><?php echo $tanggal;?></td>
				<td style="text-align:left"><?php echo $namamember."<br>".$iduser;?></td>
...
function contact_admindetail($linksub,$subdomain,$act) {
	$admin=new admin ();
	$admin->get_variable();
	$admin->setLinkSub($linksub);
	$adm=$admin->adm;
	...
	if (empty ($_GET['no'])) { $admin->notify("empty"); }
		...
		$qcontact=mysqli_query($koneksi, "SELECT judul,subdomain FROM psb_contact WHERE no='$id_contact'");
		$dcontact=mysqli_fetch_array($qcontact);
		$judul=$dcontact['judul'];
		...
		$nomor=1;
		$y=1;
		$query=mysqli_query($koneksi, "SELECT *,DATE_FORMAT(tgl,'%d-%m-%Y %H:%i:%s') as tanggal FROM psb_contact_isi WHERE id_contact='$id_contact' AND subdomain='$subdomain' ORDER BY tgl ASC");
		while($data=mysqli_fetch_array($query)) { 
			$no=$data['no'];
			$tanggal=$data['tanggal'];
			$isi=$data['isi'];
...
function contact_answer($linksub,$subdomain,$act) { 
	...
	elseif ($_POST['process']=="answer") {
		if (empty($_SESSION['kode'])) {  $admin->notify($subdomain,$linksub,"empty_session");	}
			...
			$id_contact=strip_tags($_POST['id_contact']); 
			$isi=strip_tags($_POST['isi']); $isi=str_replace('"','',$isi); $isi=str_replace("'","",$isi);
			$qcekiduser=mysqli_query($koneksi, "select iduser,subdomain from psb_contact_isi where id_contact='$id_contact'");
			...
			if ($isi=="" or $isi=="Tulis Jawaban...") { ?><div class="warning"><h3>Komentar Masih Kosong</h3>Silahkan Isi Jawaban Anda<br><a href="<?php echo $admin->getHttp();?>://<?php echo $linkmod."/act/detail/".$id_contact."/admin/";?>" title="Kembali">&laquo; Kembali</a></div><?php }
				...
				if (empty($_FILES['gambar']['tmp_name'])) {
					mysqli_query($koneksi, "INSERT INTO psb_contact_isi (id_contact,subdomain,iduser, isi, tgl) VALUES ('$id_contact','$subdomain_contack', '$subdomain', '$isi', sysdate())");
					mysqli_query($koneksi, "UPDATE psb_contact SET status='answered', tgl=sysdate() WHERE no='$id_contact'"); ?>
...
function contact_adminclosed($linksub,$subdomain,$act) { 
	$admin=new admin ();
	$admin->get_variable();
	$admin->setLinkSub($linksub);
	$adm=$admin->adm;
	...
	if (empty($_POST['process'])) {  ?><div class="warning"><h3>URL Salah</h3>Silahkan Ulangi Lagi<br></div><?php }
	elseif ($_POST['process']=="closed") {
		if (empty($_SESSION['kode'])) {  $module->notify("empty_session");	}
		else {
			if (empty($_SESSION['kode'])) { } else { unset($_SESSION['kode']); }
			$id_contact=strip_tags($_POST['id_contact']);
			mysqli_query($koneksi, "UPDATE psb_contact SET status='closed', tgl=sysdate() WHERE no='$id_contact'"); ?>
...
if ($tampil==1) {  
	...
	elseif($akses=="member"){
		if (uri(2)=="pesan" && !uri(3)) {
				contact_list($linksub,$subdomain,$link);
		}
		elseif (uri(2)=="pesan" && uri(3)=="detail" && (int) uri(4)) {
			if (empty($_POST['process'])) {
				contact_detail($linksub,$subdomain,$link,$folder,uri(4));	
			}
			elseif ($_POST['process']=="reply") {
				contact_reply($linksub,$subdomain,$link,$folder);	
			}
			elseif ($_POST['process']=="closed") {
				contact_closed($linksub,$subdomain,$link);	
			}
		}
		elseif (uri(2)=="pesan" && uri(3)=="create") {
			contact_create($linksub,$subdomain,$link,$folder);
		}
		else {
			contact_list($linksub,$subdomain,$link);
		}		
		...
		if ($paket=="free") { ?>
			...
			if ($aktif==0) { ?><h3>Fitur Tidak Aktif</h3>Maaf, Fitur ini belum aktif, karena Anda belum melakukan Pembayaran.<?php }
				...
				if (empty ($act)) {
					$act="open";
						contact_adminlist($linksub,$subdomain,$act);
					} 
					...
					elseif ($act=="detail") {
						if (empty($_POST['process'])) {
							contact_admindetail($linksub,$subdomain,$act);
						}
						elseif ($_POST['process']=="answer") {
							contact_answer($linksub,$subdomain,$act);
						}
						elseif ($_POST['process']=="closed") {
							contact_adminclosed($linksub,$subdomain,$act);
						}				
					} 
					else {
						$act="open";
