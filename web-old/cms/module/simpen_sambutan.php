	...
	elseif($akses=="admin" or $akses=="super"){
		...
		if ($paket=="free" or $paket=="basic") { ?>
			...
			if ($aktif==0){
				...
				$query=mysqli_query($koneksi, "SELECT judul,isi,gambar FROM simpen_sambutan WHERE subdomain='$subdomain'");
				...
				elseif ($_POST['proses']=="edit") {
					if (empty($_SESSION['kode'])) {?>
						<h3>Sambutan Kepala Sekolah Berhasil Disimpan</h3>Selamat, Sambutan Kepala Sekolah Berhasil Disimpan<br/><a href="<?php echo $module->getHttp();?>://<?php echo $linkhalaman;?>" title="Kembali">Kembali</a><?php
					}
					else {
						if ($_FILES['gambar']['tmp_name']=="") {
							...
							if ($jumlah==1) { mysqli_query($koneksi, "UPDATE simpen_sambutan SET judul='$judul', isi='$isi' WHERE subdomain='$subdomain'"); }
							else { mysqli_query($koneksi, "INSERT INTO simpen_sambutan (subdomain, judul, isi, tgl) VALUES ('$subdomain', '$judul', '$isi', sysdate())"); } ?>
							<h3>Sambutan Kepala Sekolah Berhasil Disimpan</h3>Selamat, Sambutan Kepala Sekolah Berhasil Disimpan<br/><a href="<?php echo $module->getHttp();?>://<?php echo $linkhalaman;?>" title="Kembali">Kembali</a><?php
						} 
						else {
							...
							elseif ($header_dimensi['0']>"1000" or $header_dimensi['1']>"1000") { ?>
								...
								$judul=strip_tags($_POST['judul']); $judul=str_replace('"','',$judul); $judul=str_replace("'","",$judul);
								$isi=str_replace("'","",$_POST['isi']); $isi=str_replace('"','',$isi);
								if ($jumlah==1) { mysqli_query($koneksi, "UPDATE simpen_sambutan SET judul='$judul', isi='$isi', gambar='$judul_baru' WHERE subdomain='$subdomain'"); }
								else { mysqli_query($koneksi, "INSERT INTO simpen_sambutan (subdomain, judul, isi, gambar, tgl) VALUES ('$subdomain', '$judul', '$isi', '$judul_baru', sysdate())"); }
